...
public class InventarioModelAssembler implements RepresentationModelAssembler<Inventario, EntityModel<Inventario>> {
    ...
    public CollectionModel<EntityModel<Inventario>> toCollectionModel(Iterable<? extends Inventario> entities) {
        CollectionModel<EntityModel<Inventario>> collectionModel = RepresentationModelAssembler.super.toCollectionModel(entities);
        collectionModel.add(linkTo(methodOn(InventarioController.class).listarMovimientosDeInventario()).withSelfRel());
